...
g = {}
with open("../Recursion/JuliaRecursion/computedG.csv") as f:
    for line in f:
...
def compute(n, k):
    return factorial2(2 * n - 1, exact=True) * sum(
        g[n][i] * k**i for i in range(len(g[n]))
    )
...
for i, n in enumerate(ns):
    ...
    plt.errorbar(
        ks,
        [numerical_data[n, k][1] for k in ks],
